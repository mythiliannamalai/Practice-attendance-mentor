...
wait = WebDriverWait(driver, 200)
disable_practice_button = wait.until(EC.element_to_be_clickable((By.XPATH, "//button[@aria-label='disable']")))
disable_practice_button.click()

yes_practice_button = wait.until(EC.element_to_be_clickable(
    (By.XPATH, "//button[normalize-space()='Yes, Disable']")
))

time.sleep(1)
# Use JavaScript click to avoid overlay/interception
driver.execute_script("arguments[0].click();", yes_practice_button)
